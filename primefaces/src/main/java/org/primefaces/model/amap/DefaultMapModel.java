...
public class DefaultMapModel implements MapModel, Serializable {
	...
	public void addOverlay(Overlay overlay) {
		if(overlay instanceof Marker) {
			overlay.setExtData(MARKER_ID_PREFIX + UUID.randomUUID().toString());
                        markers.add((Marker) overlay);
		}
		else if(overlay instanceof Polyline) {
			overlay.setExtData(POLYLINE_ID_PREFIX + UUID.randomUUID().toString());
			polylines.add((Polyline) overlay);
		}
		else if(overlay instanceof Polygon) {
			overlay.setExtData(POLYGON_ID_PREFIX + UUID.randomUUID().toString());
			polygons.add((Polygon) overlay);
		}
		else if(overlay instanceof Circle) {
			overlay.setExtData(CIRCLE_ID_PREFIX + UUID.randomUUID().toString());
	...
	public Overlay findOverlay(String id) {
		...
		for(Iterator iterator = list.iterator(); iterator.hasNext();) {
			Overlay overlay = (Overlay) iterator.next();
                        ...
                        if(overlay.getExtData().equals(id))
				return overlay;
		}
